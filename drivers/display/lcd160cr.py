...
class LCD160CR:
    ...
    def screen_dump(self, buf, ww=None, hh=None, x=0, y=0):
        if ww is None:
            ww = self.w
        if hh is None:
            hh = self.h
        if ww*2 <= 254:
            line = bytearray(ww*2 + 1)
        else:
            # split line if more than 254 bytes needed
            buflen = (ww+1)//2
            line = bytearray(buflen*2+1)
            line2 = memoryview(line)[:(ww - buflen)*2+1]
        h = len(buf) // (2 * ww)
        if h > hh:
            h = hh
        for i in range(h):
    ...
    def sprite(self, buf, w, h, x, y, bpp=16):
        if not x:
            x = 0
        if not y:
            y = 0
        ...
        l = (w * h * (bpp & 0x1f)) >> 3
        self._fcmd2b('<BBHBBB', 0x70, l+2, bpp, w, h)
        n = len(buf)
        if n > l:
            n = l
        ar = memoryview(buf)
